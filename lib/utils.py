...
def Random_proj(img1,img2,crop_size=192,ratio=0.2,scale=0.2,rotation=180):
    half_cs = crop_size//2
    if len(img1.shape)<4:
    ...
    #Corners mapping (affine)
        ...
        angle = (torch.randn(1)*180).clamp(min=-rotation,max=rotation)
        #scale
        scale = np.random.uniform(0.5, 3)
        scale = scale * torch.ones((1, 2))
        # scale = (1-torch.rand(1)*scalce)*torch.ones((1,2))
        #calulate the homograph
        M_suf = KGT.get_rotation_matrix2d(torch.FloatTensor([[w/2,h/2]]),angle,scale)
        M_suf = torch.cat([M_suf,torch.FloatTensor([[[0,0,1]]])],dim=1)
        M = KGT.get_perspective_transform(points_src, points_dst)@M_suf #Homography
    ...
    return img1_crop,img2_crop,warp_grid, angle
