...
class Adapter_MMnet(BaseNet):
    def __init__(self, mchan=4, **kw):
        super(Adapter_MMnet, self).__init__()
        t = BaseNet()
        tt = nn.ModuleList([])
        ops = nn.ModuleList([])
        ops.append(t.MakeBlk(8 * mchan))
...
class ConditionalEstimator(nn.Module):
    def __init__(self, channels=128) -> None:
        ...
        self.preconv = nn.Sequential(nn.Conv2d(channels, 64, kernel_size=3, padding=1, bias=False),
...
class REMM(nn.Module):
    def __init__(self):
        super(REMM, self).__init__()
        self.ada1 = Adapter_MMnet()
        self.ada2 = Adapter_MMnet()
        self.enc = Encoder()
        self.det = Superhead()
